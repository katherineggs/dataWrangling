...
to_remove <- c(stopwords(language = "en"),
               ...
               c(1:1000),
               c("imagen","sticker","image","omitida","omitido","jajaja","jajajaja", "jaja"),
               url_regex)
...
#######Diversidad lexica########### palabras dif
chat %>%
  unnest_tokens(input = text,
                output = word) %>%
  filter(!word %in% to_remove) %>%
...
chat  %>% 
  mutate(day  = date(time)) %>% 
  # filter(str_detect(chat$text,"o sea")) %>% 
  group_by(author) %>% 
  count(day) %>%  
